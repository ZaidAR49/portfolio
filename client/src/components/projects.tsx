...
{projects.map((project, index) => (
  ...
  <div className="absolute inset-0 flex items-center justify-between -mx-8 sm:-mx-12 lg:-mx-16">
    <div onClick={()=>{handleImageChange("prev",index); console.log(index)}} className="text-2xl sm:text-3xl text-white hover:scale-125 transition cursor-pointer select-none">❮</div>
    <div onClick={()=>{handleImageChange("next",index); console.log(index)}} className="text-2xl sm:text-3xl text-white hover:scale-125 transition cursor-pointer select-none">❯</div>
  </div>
</div>
...
<div className="header-secondary text-2xl sm:text-3xl lg:text-4xl">{project.title} </div>

<p className="text-base sm:text-lg lg:text-xl">{project.description}</p>
 <div className="bg-black text-white p-4 sm:p-6 max-w-xl border border-gray-800">
      <h2 className="text-sm sm:text-base font-semibold mb-4">PROJECT INFO</h2>
